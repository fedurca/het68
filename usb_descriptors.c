...
static uint8_t const desc_configuration[] = {
  ...
  // UAC2 CS_AC_HEADER: bLength, bDescType, bDescSubtype, bcdADC(2LE), bCategory, wTotalLength(2LE), bmControls
  LEN_CS_AC_HEADER, 0x24, 0x01, 0x00, 0x02, 0x03, (uint8_t)(AC_CS_TOTAL & 0xFF), (uint8_t)(AC_CS_TOTAL >> 8), 0x00,
  // ---- Clock Source Entity ----
  // bmControls=0x05: bits[1:0]=01 (freq read-only), bits[3:2]=01 (validity read-only)
  LEN_CS_CLK_SRC, 0x24, 0x0A, ID_CLK, 0x01, 0x05, 0x00, 0x00,
  // ---- Input Terminal ----
  // bmChannelConfig=0x0000003F: FL+FR+FC+LFE+BL+BR (6 standard channels)
  LEN_CS_IT, 0x24, 0x02, ID_IT, 0x01, 0x02, 0x00, ID_CLK, AUDIO_N_CHANNELS, 0x3F,0x00,0x00,0x00, 0x00, 0x00, 0x00, 0x00,
  // ---- Feature Unit ----
  LEN_CS_FU, 0x24, 0x06, ID_FU, ID_IT,
  0x00, 0x00, 0x00, 0x00, // Master: no exposed controls (avoids TinyUSB 0.18 FU GET bug)
  0x00, 0x00, 0x00, 0x00, // Ch1: no per-channel controls
  0x00, 0x00, 0x00, 0x00, // Ch2: no per-channel controls
  0x00, 0x00, 0x00, 0x00, // Ch3: no per-channel controls
  0x00, 0x00, 0x00, 0x00, // Ch4: no per-channel controls
  0x00, 0x00, 0x00, 0x00, // Ch5: no per-channel controls
  0x00, 0x00, 0x00, 0x00, // Ch6: no per-channel controls
  0x00,                   // iFeature
  // ---- Output Terminal ----
  ...
  16, 0x24, 0x01, ID_OT, 0x00, 0x01, 0x01, 0x00, 0x00, 0x00, AUDIO_N_CHANNELS, 0x3F,0x00,0x00,0x00, 0,
  // ---- Type I Format (16-bit) ----
  6, 0x24, 0x02, 0x01, AUDIO_SAMPLE_BYTES, 16,
  // ---- Standard ISO IN Endpoint ----
  // bmAttributes=0x0D: Isochronous + Synchronous + Data (no feedback endpoint needed)
  7, TUSB_DESC_ENDPOINT, EP_ADDR_AUDIO_IN, 0x0D, (AUDIO_PACKET_SIZE & 0xFF), (AUDIO_PACKET_SIZE >> 8), 0x01,
  // ---- CS ISO IN Endpoint ----
