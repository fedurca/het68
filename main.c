...
int main(void)
{
  ...
  // LED na GP25 (Pico/Pico 2)
  const uint LED_PIN = 25;
  gpio_init(LED_PIN);
  gpio_set_dir(LED_PIN, GPIO_OUT);
  gpio_put(LED_PIN, 0);

  // 5 bliknutí/s => perioda 200 ms => toggle každých 100 ms
  absolute_time_t next_toggle = make_timeout_time_ms(100);
  ...
  while (true) {
    ...
    if (absolute_time_diff_us(get_absolute_time(), next_toggle) <= 0) {
      ...
      next_toggle = delayed_by_ms(next_toggle, 100); // bez driftu
    }
  }
  return 0;
}
